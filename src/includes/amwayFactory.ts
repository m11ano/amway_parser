...
const Factory = {
            ...
            try {
              ...
              let engine = (subCarts: Array<any>) => {
                for (let item of subCarts) {
                  orderDataItem.subOrders.push(
                    this.parseOrderFromData(item, "KZ")
                  );

                  if (
                    typeof item.subCarts == "object" &&
                    item.subCarts.length > 0
                  ) {
              ...
              if (
                typeof data.orderData.subCarts == "object" &&
                data.orderData.subCarts.length > 0
              ) {
  ...
  parseOrderFromData(data: any, postfix: string = ""): orderDataItem {
    if (
      typeof data.orderedBy != "object" ||
      typeof data.orderedBy.account != "object"
    ) {
    ...
    let customer: orderDataItemCustomer = {
      id:
        typeof data.orderedBy.account.code == "string"
          ? data.orderedBy.account.code
          : "",
      name: typeof data.orderedBy.name == "string" ? data.orderedBy.name : "",
      sponsorId:
        typeof data.orderedBy.account.sponsorAboNumber == "string"
          ? data.orderedBy.account.sponsorAboNumber
          : "",
    };
    ...
    if (typeof data.orderedBy.phones == "object") {
      ...
      for (let phone of data.orderedBy.phones) {
        phones.push(
          `+${phone.phoneCntryCd}${phone.phoneAreaCd}${phone.phoneLocalNum}`
        );
      }
    ...
    if (typeof data.orderedBy.defaultBillingAddress == "object") {
      ...
      if (typeof data.orderedBy.defaultBillingAddress.region == "object") {
        customer.address +=
          ", " + data.orderedBy.defaultBillingAddress.region.name;
      }
    ...
    let result: orderDataItem = {
      ...
      customer: customer,
      goods: [],
      discount: Number(data.totalDiscounts.value),
      sum: Number(data.subtotalWithoutProductDiscount.value),
      totalSum: Number(data.totalPrice.value),
    };

    result.sum = result.totalSum + result.discount;
    ...
    if (typeof data.entries == "object") {
      ...
      for (let good of data.entries) {
        if (
          typeof good.component == "object" &&
          typeof good.component.lynxOrderingNumber == "string"
        ) {
...
export { orderData, orderDataItem, orderDataItemCustomer, orderDataItemGood };
