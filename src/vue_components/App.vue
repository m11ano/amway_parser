        ...
        <template v-else>
          <div class="app">
            ...
            <div class="block_2" v-if="orderData !== undefined">
              <template v-for="(order, id) in orderDataForLoop" :key="id">
                <div :class="{'hide' : order.use === false}">
                  <div class="top">
                    <div class="left">
                      <div class="i1"><template v-if="id == 0">Заказ:</template><template v-else>Подзаказ:</template> <span>{{order.data.id}}</span></div>
                      <div class="i2">№ {{order.data.customer.id}} <span>{{order.data.customer.name}}</span></div>
                    </div>
                    <template v-if="id != 0">
                    <div class="right">
                      <div class="title">Использовать:</div>
                      <div class="input"><input type="checkbox" v-model="order.use" /></div>
                    </div>
                    </template>
                  </div>
                  <div class="goods">
                  ...
                  <div class="sum">
                    Итого: <span class="value">{{coolNumber(countOrderSum(order.data.goods))}}</span> ₸
                  </div>
                </div>
              </template>
            </div>
            <div class="block_3" v-if="orderData !== undefined">
              ...
              <div class="block_error" v-if="(typeof block_3_Error == 'string' && block_3_Error.length > 0) || typeof block_3_Error == 'object'">{{block_3_Error}}</div>
              <div class="block_result" v-if="block_3_Result.length > 0">{{block_3_Result}}</div>
...
export default defineComponent({
  data() {
    return {
      ...
      orderDataForLoop : [] as {use : boolean, data : orderDataItem}[] | undefined,
